...
io.sockets.on('connection', function(socket) 
{
	...
	socket.on("loginUser", function(username)
	{
		...
		socket.broadcast.emit("refreshChat", "conectado", socket.username);
	...
	{
		socket.broadcast.emit("refreshChat", "msg", {from:socket.username, msg: message});
	});
	...
	socket.on('chat', function(to, message) 
	{
		...
		clients[to].emit("chat", "msg", {from:socket.username, msg: message});
	});
	...
	socket.on("disconnect", function()
	{
		...
		socket.broadcast.emit("refreshChat", "desconectado", socket.username);
	});
});
